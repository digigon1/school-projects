...
test_folder = None
test_folder = 'models/2020-02-14T00:43:30.757880'

prev_model = None
# prev_model = 'models/2020-02-14T00:43:30.757880/weights.218-0.28609.hdf5'
# done_epochs = 218
...
split_point = int(len(X_array[0]) * 0.8) # Memory limited to 0.87 instead of usual 0.8
...
seq = CustomSequence(X_array, Y_array, time_window, batch_size=int(len(X_array[0]) / 800)) # Fixed ~(200 * 5) iterations

# train_features = np.array([X_train[i - time_window : i + 1] for i in range(time_window, len(X_train))])

print('====== Creating test_features ======')
test_features = np.array([X_test[i - time_window : i + 1] for i in range(time_window, len(X_test))])
...
model.add(CuDNNLSTM(units=outputs, input_shape=(time_window + 1, feature_count), return_sequences=False))

# opt = RMSprop(0.001)
opt = SGD()
model.compile(loss='mean_squared_error', optimizer=opt, metrics=['accuracy', coeff_determination])
...
print('====== Creating test_y ======')
test_y = np.array(Y_test[time_window:])
if test_folder is not None:
	...
	for file in files:
		if file.find('weights') != -1:
			...
			score = model.evaluate(test_features, test_y, verbose=1)
